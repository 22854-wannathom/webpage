(() => {
  ...
  if (!document.querySelector(link[href="${CSS_FILE}"])) {
  ...
  function createPetals() {
    ...
    const count = Math.min(28, Math.max(14, Math.floor(window.innerWidth / 45)));
  ...
  function hoverInteraction() {
    ...
    interactive.forEach((element) => {
      element.classList.add("floral-interactive");
    });
  }
  ...
  function softGlow() {
    ...
    headings.forEach((heading) => {
      heading.classList.add("floral-soft-glow");
    });
  }
